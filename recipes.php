...
) t WHERE  recipe_id1 = (SELECT (recipe_id) FROM (  SELECT  recipe_id FROM recipes ORDER BY (RAND()) ) t LIMIT 1)";
       ...
       <ul>
         <li><?php echo $row2['qty'] . " " . $row2['unit'] . " " . $row2['ingredient']; ?></li>
				...
				<?php if ( isset($_SESSION['logged_in']) && $_SESSION['logged_in'] ) : ?>
				 <button type="button" class="btn btn-primary btn-lg" OnClick="location.href='saveRecipe.php?recipe_id=<?php echo $row1['recipe_id1'] ?>'">Save for later</button>
			 <?php endif; ?>
     </div>
   </div>
